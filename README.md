...
<p align="center">
  ...
  <a href="https://mistral.ai/">
    <img src="https://img.shields.io/badge/AI-Mistral%20AI-FF7000?logo=mistral&logoColor=white" alt="Mistral AI" />
  </a>
</p>
...
<table>
  ...
  <tr>
    ...
    <td>Leverages Mistral AI to generate 5 categories of career resources in parallel.</td>
  </tr>
  <tr>
  ...
  <tr>
    <td><b>📖 Google Books Verification</b></td>
    <td>AI-suggested books are enriched with verified metadata (author, edition, publisher) and stable, permanent Google Books URLs. No more broken links.</td>
  </tr>
  <tr>
...
## 📖 Google Books Verification (Automatic)

Book recommendations start as AI-generated suggestions from **Mistral AI**, but are then **automatically verified and enriched** via the Google Books API:

| | Without Enrichment | With Google Books |
|---|---|---|
| **Metadata** | AI-generated author/edition (may be inaccurate) | Verified from Google Books database |
| **URLs** | May break over time | Permanent `books.google.com` links |
| **Coverage** | All books get a URL | Fallback to Google search if no match |
| **Speed** | Fast (~2-3s) | +1-2s per book for enrichment |
| **Cost** | Free | Free (1,000 req/day without key, higher with key) |
| **Setup** | None — works out of the box | Optional `GOOGLE_BOOKS_API_KEY` for higher quotas |

**How it works:**
1. Mistral AI generates 3 book recommendations relevant to your field and region
2. Each book title is searched against Google Books API (`{title} {field}` query)
3. The best match enriches the record with verified author, publisher, edition year, and a permanent URL
4. If Google Books returns no match, the original AI data is kept with a generic Google search fallback URL

**Why this matters:**
- No more "link rot" — Google Books URLs are permanent and maintained by Google
- Legally clean — links to previews, purchase pages, or library borrow links
- Better user experience — accurate author names, edition years, and publisher info

---

## 🎬 Gemini Video Search (Optional)

By default, video recommendations are generated by **Mistral AI**, which can suggest titles that don't actually exist on YouTube. Enable **Google Gemini** to get real, currently available videos:

| | Without Gemini | With Gemini |
|---|---|---|
| **Source** | Mistral AI (generative) | Gemini 2.0 Flash + Google Search |
| **Accuracy** | May suggest non-existent videos | Verified, real YouTube URLs |
| **Speed** | Fast (~2-3s) | Slightly slower (~5-8s) |
| **Cost** | Mistral API tier | Free tier available on Google AI Studio |
| **Setup** | Add `MISTRAL_API_KEY` to `.env.local` | Add `GOOGLE_GEMINI_API_KEY` to `.env.local` |
...
4. If Gemini fails or is unavailable, it gracefully falls back to Mistral AI
...
> Get an API key at [console.mistral.ai/api-keys](https://console.mistral.ai/api-keys/).
...
> Get a free key at [aistudio.google.com/apikey](https://aistudio.google.com/apikey). Without it, video recommendations are AI-generated by Mistral AI (may suggest non-existent videos).

**(Optional)** Add a Google Books API key for higher request quotas:

```env
GOOGLE_BOOKS_API_KEY=AIzaSy...
```

> Enable the "Books API" at [console.cloud.google.com/apis/credentials](https://console.cloud.google.com/apis/credentials). Without a key, the API works for free at 1,000 requests/day. With a key, quotas increase significantly.
...
│   ├── google-books.ts           ← Google Books API verification + enrichment
...
- [x] **Google Books verification** — AI-suggested books enriched with verified metadata + stable Google Books URLs
...
- **[Mistral AI](https://mistral.ai/)** — AI inference platform
- **[Google Books API](https://developers.google.com/books)** — Verified book metadata and permanent URLs
