...
const TABS: { key: Tab; label: string }[] = [
  ...
  { key: "professional_titles", label: "Titles & Salary" },
];
...
export function RecommendationsDashboard({ payload }: { payload: RecommendationPayload | null }) {
  ...
  const dataMap: Record<Tab, any[]> = { books, videos, projects, online_resources, professional_titles };
        ...
        {TABS.map(({ key, label }) => (
          <section key={key} className="space-y-2">
            ...
            <ul className="space-y-2">
              {(dataMap[key] || []).map((item: any, i: number) => (
      ...
      {professional_titles && (
        <div className="mt-2 border-t border-neutral-800 pt-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {professional_titles.map((t: any, i: number) => (
